...
function CartWidget() {
  ...
  return (
    <NavLink to={`/cart`} > <button type="button" className="inline-flex relative items-center p-3 
      text-sm font-medium text-center text-white  rounded-lg  focus:ring-4 focus:outline-none focus:ring-red-300 ">
      ...
      {cartLength > 0 ?
        <div className="inline-flex absolute -top-2 -right-2 justify-center items-center w-6 h-6
          text-xs font-bold text-white bg-red-500 rounded-full border-1 border-black dark:border-gray-900">{cartLength}</div>
        : <></>}
    </button></NavLink>
  );
}
